...
# 0.16 BACKPORT NOTE. This module was written on `oellm/v0.19` and is kept
# line-for-line identical to it except where megatron-core 0.16 has no
# equivalent API. Every such place is marked `0.16:`. There are three:
#
#   1. `param_is_not_gtp_duplicate` does not exist (0.19 introduced it with
#      global-tensor-parallel rematerialisation, which 0.16 has no concept of),
#      so the gradient dedup filter is the TWO-filter set that 0.16's own
#      `clip_grads.py:220` uses. `diag/grad_norm/total_check` validates this.
#   2. `param_is_not_tensor_parallel_duplicate` takes no group arguments on
#      0.16 — it reads the process groups off the global `parallel_state`.
#   3. `vocab_parallel_logsumexp` does not exist; 0.16 only exposes log Z
#      through the cross-entropy autograd Function's `return_logsumexp`.
#      `_vocab_parallel_logsumexp` below is the no-grad equivalent.
...
def _vocab_parallel_logsumexp(vocab_parallel_logits, global_max, tp_group):
    """0.16: log Z per token over a vocabulary split across tensor-parallel ranks.

    `oellm/v0.19` gets this from `core.tensor_parallel.cross_entropy.
    vocab_parallel_logsumexp`, a differentiable autograd Function added there for
    the LM-head z-loss. On 0.16 log Z is reachable only through the
    cross-entropy Function's `return_logsumexp` path, which needs the targets and
    builds a graph. Neither is wanted here: this runs inside a forward hook under
    `torch.no_grad()` and only reports a number.

    Same arithmetic as the 0.19 helper, and the same as 0.16's
    `_VocabParallelCrossEntropy.forward` (`cross_entropy.py:124-163`) — subtract
    the GLOBAL per-token max before exponentiating, sum the local shard, all-reduce,
    then `log(sum) + max`. Passing `global_max` in rather than recomputing it saves
    the second MAX all-reduce: the caller has already reduced it for `token_max_mean`.

    Args:
        vocab_parallel_logits: [s, b, V/TP], this rank's vocabulary shard.
        global_max: [s, b] float32, per-token max already reduced over `tp_group`.
        tp_group: the tensor-parallel group to sum the partial exponentials over.

    Returns:
        [s, b] float32 per-token log-partition over the FULL vocabulary.
    """
    shifted = (vocab_parallel_logits.float() - global_max.unsqueeze(-1)).exp()
    sum_exp = shifted.sum(dim=-1)
    torch.distributed.all_reduce(sum_exp, op=torch.distributed.ReduceOp.SUM, group=tp_group)
    return sum_exp.log() + global_max
...
class TrainingDiagnostics:
    ...
    def _build_grad_map(self, model, optimizer, slot_of: Dict[int, int]):
        ...
        """
        def keep(param) -> bool:
            """The dedup filters Megatron's own global grad norm applies.

            Both are load-bearing; dropping either inflates the result by exactly
            the factor that axis replicates.
            ...
            0.16: on `oellm/v0.19` there is a THIRD filter,
            `param_is_not_gtp_duplicate`. It does not exist here and must not be
            faked: it deduplicates global-tensor-parallel rematerialisation, a
            0.19-only feature, so on 0.16 there is no such replication to remove.
            This matches 0.16's own `_get_main_grads_for_grad_norm`
            (`core/optimizer/optimizer.py:155-156`) and `clip_grads.py:220-221`,
            which likewise apply exactly these two. `diag/grad_norm/total_check`
            is the check that this is right — it must equal the `grad-norm`
            Megatron computes independently.
            """
            return param_is_not_shared(param) and param_is_not_tensor_parallel_duplicate(param)
    ...
    def _install_logit_hook(self, model):
        ...
        THESE LOGITS ARE PRE-SOFTCAP. `--final-logit-softcapping` is applied in
        `GPTModel.forward` AFTER `self.output_layer(...)` returns
        (`core/models/gpt/gpt_model.py:613`), so a forward hook on the output
        layer necessarily sees the uncapped values — the same on 0.16 as on
        `oellm/v0.19`. That is the useful signal (an uncapped logit above `c`
        means the cap is saturated and the tanh is throttling the gradient) but
        it is NOT what the loss consumes, so do not read `diag/logits/max`
        against the cap as if the two were the same quantity.
        """
        dev = torch.cuda.current_device()
        ...
        def hook(module, inputs, output):
            ...
            with torch.no_grad():
                ...
                logz = _vocab_parallel_logsumexp(x, tok_max, tp_group)  # [s, b], full vocab
