...
def track_moe_metrics(
    ...
    for name, loss_list in aux_losses.items():
        ...
        if writer is not None:
            ...
            if wandb_writer:
                wandb_writer.log(
                    {f"router-aggregates/{name}": loss_list.sum() / num_moe_layers}, iteration
                )
                if per_layer_logging:
                    wandb_writer.log(
                        {
                            f"router-layers/{name}_layer_{i}": loss
                            for i, loss in enumerate(loss_list.tolist())
                        },
                        iteration,
                    )
...
def save_to_expert_utilization_tracker(
    ...
    logging happen once per optimizer step inside track_moe_metrics().
...
def save_to_router_stats_tracker(
    ...
    Distributed reduction and logging happen once per optimizer step inside track_moe_metrics().
